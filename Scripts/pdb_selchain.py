...
Modified original script in lines 133-144 to adhere to MEGADOCK output format
"""
...
def run(fhandle, chain_set):
    ...
    modelrecords = ('MODEL', 'END')
    salts = ['Na+', 'K+', 'Cl-']
    for line in fhandle:
        if line.startswith(records):
            if line.startswith('TER') and (len(line) < 21):
                yield line
                continue
            elif line.startswith(modelrecords) and (len(line) > 21):
                continue
            elif line[21] not in chain_set:
                continue
            elif any(salt in line for salt in salts):
                continue
        yield line
